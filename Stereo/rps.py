...
class RPS(object):
    ...
    def save_normalmap_as_png(self, filename):
        """
        Saves normal map as png format
        :param filename: filename of a normal map
        :return: None
        """
        normal_map_big = self.N
        normal_map = cv.resize(normal_map_big, (1000, 1000))
        print(len(normal_map))
        normal_map_scaled = (normal_map* 255).astype(np.uint8)
        print(filename)
        cv.imwrite(filename, normal_map_scaled)
