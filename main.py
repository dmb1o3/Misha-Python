...
from calibrate_lights import calibrate_light
...
def run():
 ...
 #   calibrate_light(directory, 4)
    ...
    # Update layout
    fig.update_layout(title='Original Depth Map vs Fitted Polynomial Surface', scene_zaxis=dict(range=[min_z, max_z*4]), autosize=True)
    fig.show()
    # Display the corrected depth map
    fig2 = go.Figure()
    fig2 = go.Figure(data=[go.Surface(z=corrected_depth_map, colorscale='gray')])
    fig2.update_layout(title='Corrected Depth Map', scene_zaxis=dict(range=[min_z, max_z]), autosize=True)
    fig2.show()
    # Display Original depth map
    fig3 = go.Figure()
    fig3 = go.Figure(data=[go.Surface(z=depth_map, colorscale='gray')])
    fig3.update_layout(title='Depth Map', scene_zaxis=dict(range=[min_z, max_z]), autosize=True)
    fig3.show()
